...
export default function SignupPage() {
  ...
  const handleEmailSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          data.message || "Database synchronization failed"
        );
      }
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      // Use Google Name as Company Name
      ...
      // Trigger Login for Cookie Session
    ...
    <div className="relative min-h-screen bg-[#050505] flex justify-center items-center px-4 overflow-hidden">
      <div className="relative w-full max-w-md animate-in fade-in zoom-in-95 duration-500">
        ...
        <div className="bg-neutral-900/50 border border-neutral-800/60 backdrop-blur-xl rounded-3xl p-8 shadow-2xl space-y-5">
          {/* Error */}
          {error && (
            <div className="text-rose-400 bg-rose-500/10 p-3 rounded-xl text-sm flex items-center gap-2">
              <AlertCircle size={16} />
              {error}
            </div>
          )}
          ...
          {/* Success */}
          {success && (
            <div className="text-emerald-400 bg-emerald-500/10 p-3 rounded-xl text-sm flex items-center gap-2">
          ...
          <form onSubmit={handleEmailSignup} className="space-y-4">
            ...
            <input
              name="mobile"
              type="tel"
              placeholder="Mobile Number (Optional/Info)"
              onChange={handleChange}
              className="w-full bg-neutral-950/50 border border-neutral-800 focus:border-amber-500/50 outline-none rounded-2xl px-4 py-3 text-white transition-colors"
            />
            ...
            <div className="relative">
              <input
                ...
                required
                minLength={6}
                className="w-full bg-neutral-950/50 border border-neutral-800 focus:border-amber-500/50 outline-none rounded-2xl px-4 py-3 pr-12 text-white transition-colors"
              />
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-amber-500 hover:bg-amber-400 disabled:bg-neutral-800 disabled:text-neutral-500 text-black py-3.5 rounded-2xl font-bold transition-colors"
            >
          ...
          <button
            ...
            disabled={loading}
            className="w-full bg-white hover:bg-gray-100 text-black py-3.5 rounded-2xl font-bold transition-colors flex items-center justify-center gap-2"
          >
